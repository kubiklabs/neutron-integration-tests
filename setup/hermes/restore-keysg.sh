...
$HERMES_BINARY --config $CONFIG_DIR keys add --key-name testkey_2 --chain gaia-test-2 --mnemonic-file <(echo "record gift you once hip style during joke field prize dust unique length more pencil transfer quit train device arrive energy sort steak upset")
sleep 5
